...
def main():
    ...
    # Track and save the best validation model by SSIM.
    # Only the network weights are saved as net_g_best.pth; no best.state is saved,
    # so automatic resume will not be affected.
    best_ssim = -1.0
    best_iter = -1
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            # if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):
                ...
                log_vars.update(current_log)
                msg_logger(log_vars)

                # ------------------------------
                # Save best model by validation SSIM
                # ------------------------------
                current_ssim = None
                for metric_name, metric_value in current_log.items():
                    if 'ssim' in metric_name.lower():
                        try:
                            current_ssim = float(metric_value)
                        except Exception:
                            current_ssim = float(metric_value.item())
                        break
                ...
                if current_ssim is not None:
                    if current_ssim > best_ssim:
                        ...
                        logger.info(
                            f'New best model at iter {best_iter}: SSIM={best_ssim:.6f}. '
                            f'Saving net_g_best.pth.'
                        )

                        if opt['rank'] == 0:
                            # Save network weights only. Do NOT save best.state, otherwise
                            # the automatic resume code may try to parse best.state as an integer.
                            model.save_network(model.net_g, 'net_g', 'best')
                            best_info_path = osp.join(opt['path']['experiments_root'], 'best_model_info.txt')
                            with open(best_info_path, 'w', encoding='utf-8') as f:
                                f.write(f'best_iter: {best_iter}\n')
                                f.write(f'best_ssim: {best_ssim:.6f}\n')
                else:
                    logger.info('SSIM metric was not found in current log; skip best-model saving.')
    ...
    if opt.get('val') is not None:
        ...
        # Also check the final validation result for best SSIM.
        current_log = model.get_current_log()
        current_ssim = None
        for metric_name, metric_value in current_log.items():
            if 'ssim' in metric_name.lower():
                try:
                    ...
                    current_ssim = float(metric_value.item())
                break
        if current_ssim is not None and current_ssim > best_ssim:
            best_ssim = current_ssim
            best_iter = current_iter
            logger.info(
                f'New best model at final validation iter {best_iter}: '
                f'SSIM={best_ssim:.6f}. Saving net_g_best.pth.'
            )
            if opt['rank'] == 0:
                model.save_network(model.net_g, 'net_g', 'best')
                best_info_path = osp.join(opt['path']['experiments_root'], 'best_model_info.txt')

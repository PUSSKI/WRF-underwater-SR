...
def calculate_external_rgb_ssim(img1,
                                img2,
                                crop_border=0,
                                input_order='HWC',
                                test_y_channel=False):
    """RGB 2D SSIM compatible with Image-quality-measure-method/imgqual_utils.py.

    This metric computes SSIM per channel with scipy.ndimage.gaussian_filter
    and averages channels. It is useful for comparing with external scripts
    that do not use this repository's custom 3D SSIM.
    """
    assert img1.shape == img2.shape, (
    ...
    if test_y_channel:
        img1 = to_y_channel(img1)[..., 0]
        img2 = to_y_channel(img2)[..., 0]

    def _compute_2d_ssim(x, y):
        k1, k2 = 0.01, 0.03
        sigma = 1.5
        win_size = 5
        max_value = 1.0 if max(x.max(), y.max()) <= 1 else 255.0
    ...
    if img1.ndim == 2 or test_y_channel:
        return _compute_2d_ssim(img1, img2)
    return float(np.mean([
        _compute_2d_ssim(img1[..., ch], img2[..., ch])
